...
This is the **Codex-side `book` skill**. The full `book` pipeline (write, sniff, review, coldread, …) is a Claude Code skill; under Codex the `book` skill exposes a single subcommand, `judge` — the cross-model judge lane of the parallel-write merge phase. Invoke it as `/book judge <manifest-path> <output-path>`; the first argument is the subcommand (`judge`), the rest are the judge's two paths.

This command ranks N parallel chapter drafts of the same outline beat against a brilliance-oriented rubric, in a model-agnostic way. **Output is a strict JSON file written to the path passed as the second judge argument — no prose, no commentary, no explanations outside the JSON inside the file.** Aggregation downstream parses the file with `jq` and will fail on extraneous content.
...
`<output-path>` is the absolute path of a file you must create (or overwrite) with the JSON described under "Output schema" below. Do not print the JSON to stdout — write it to the file using your write tool. After writing, you may print a one-line status message confirming the file path and byte size; nothing else.
...
`judge_id` and `model` are hardcoded above — emit them verbatim. They identify the codex/gpt-5.5 lane of the 4-judge ensemble (per the user's pipeline config in `~/.codex/config.toml`).
