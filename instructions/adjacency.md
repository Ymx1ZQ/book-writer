...
Every other analyst reads ONE chapter (against canon, against itself, against the reader's single-point memory). This pass reads a **window of consecutive chapters together**, catching the three defects that only exist *between* chapters — the ones a per-chapter QC matrix is structurally blind to:
...
3. **Dramatic-irony legibility** — the project's reader-journey design declares an irony or reader-knowledge state spanning the pair, but the prose fails to make it legible to a first-time reader, so the planned irony is inert.

These are the exact defects a human cold-read of Book-1 ch01–ch04 surfaced (project Phase 43 / 44).
...
`motif` tracks whether a *symbol* stays consistent across chapters; this tracks whether the *shape, the voices, and the reader's intended knowledge-state* work across the seam.
...
2. `plot/reader-journey.md` — the per-chapter **New Knowledge / Active Beliefs / Dramatic Irony** rows for the window: what a first-time reader is *supposed* to know and what irony is *supposed* to be live across the pair. **The legibility yardstick for class (c).**
...
**Graph-assisted inputs (optional — see `instructions/graph-recall.md`):** if `graphify-out/graph.json` exists in the project root AND the freshness gate passes, run the granularity probe first: `graphify query "reader-journey state of <book> ch<NN>" --budget 4000` for the target chapter. Only if the probe returns a **per-chapter node** (a companion re-extraction ships that granularity project-side, but the probe gates regardless), replace inputs 2, 4, and 5 with two queries (answer mode):
...
You have read `reader-journey.md`, so you also know which seams are *meant* to rhyme (a dramatic irony, a cross-caste mirror) — and there your job is not "is this repetitive?" but **"does the prose deliver the charge that makes the repetition worth it?"**
...
- **Gate (load-bearing rhyme):** before flagging, check `reader-journey.md` + `echo-choreography.md`. If the parallel *serves* a declared irony or a designed cross-level/cross-caste mirror, the rhyme is **intended** — NOT a shape-repetition defect; route any problem to class (c) instead (the rhyme is fine, the *charge* may be missing). A rhyme is a class-(a) defect only when **inert** — repetition with no design reason.
...
- Identify the bridge the irony rests on (a shared entity / location / code / object named in both chapters). Is it salient enough in BOTH that a first reader connects them, or buried (named once, unsignposted)?
...
5. **Pre-step archive:** SMELL.md is shared — append only (do not overwrite). If a prior adjacency audit section from this cycle exists, replace that section only.
...
- **Idiolect: theme shared, words distinct.** Flag the same *wording*, never the same *theme*. The fix differentiates phrasing; it does not delete the shared device.
- **Two chapters is the unit.** Do not judge a whole book's structure here; the window is small by design so the judgment stays grounded in what a reader actually carries across one seam.
