...
- **Declared exception:** files the book's outline header promotes into the always-loaded set (§Context Tags) are loaded by authorial declaration and are not re-judged here.

The register is a POV constraint on what the writer is allowed to know, so it outranks any pointer aimed the other way: a barred file is reported, never auto-added, whatever a beat (2.6.a) or a tracker row (2.6.c) says. Distinct from the tonal Register lock below, which constrains prose, not file loading.
...
1. **Em-dash glosses:** count them against the POV character's cap in their sheet's narrator boundaries. Over the limit, cut the weakest — a period, or the sentence restructured without the gloss.
2. **Explanation sentences:** every sentence that follows a strong image or action and explains what it meant. Delete it, then read the paragraph without it: if the meaning is clear, the sentence was redundant. Target: ZERO.
3. **"The way" count:** if >3 occurrences, rewrite at least half — "like," "as," direct statement, or no simile.
4. **Tic captions:** check the state file's Tic Introductions section. A tic introduced in a previous chapter loses its explanation. Gesture only.
5. **Ending type:** if the final paragraph is a character standing or sitting still, processing, check the plan's ending type. Type J (contemplative) with 2 already used in this book: REWRITE to F, G, H, or I.
6. **Aphorisms in narration:** sentences that could appear on a book jacket without context. Cut or bury inside a longer thought. Max 1 in narration. In dialogue, acceptable ONLY if the character's voice profile specifies they speak that way.
7. **Vocabulary check:** read voice-samples.md Vocabulary Evolution. If the narration uses words or constructions the POV character does not have at this chapter range, rewrite in the register they do have.
8. **Metaphor check (Rule 1 + Rule 9):** grep narration (not dialogue) for `like a`, `as if`, `as though`, `was a [abstract noun]`. Rewrite each as a physical, concrete image — synecdoche, metonymy, precise detail, negative space. Target: **0** metaphors in narration. Dialogue metaphors only where the voice-sample profile specifies (e.g., Mira in manifestos, Anyuk in proverbs).
9. **Adverbial dialogue tags (Rule 15):** count variant tags replacing `said`. EN: `muttered, whispered, exclaimed, retorted, gasped, snapped, hissed, breathed`. IT: `mormorò, sussurrò, esclamò, ribatté, ansimò, sibilò`. Total >2: rewrite the weakest half as action beats (e.g., `He set the mug down. "I know."`).
10. **Chapter opening check (Rule 21):** the first 150 words must carry a concrete image + tension — a question, a wrongness, a decision. Summary, orientation or a stative verb (`Noah was X`, `It was Y`, `The Z did W as usual`) means rewriting the opening in medias res.
11. **Chapter closing check (Rule 22):** the final paragraph must be ≤2 sentences resolving as a single image or line. A summary or contemplative wrap-up >2 sentences cuts to the most resonant single line. Never end with narrator-delivered significance.
12. **Transition phrases (Rule 13):** grep for `later that day, a few hours later, after that, eventually, meanwhile, più tardi, dopo, in seguito, frattanto, eventualmente`. Each hit becomes a white-space scene break + an in-medias-res opening.
13. **Object permanence (Rule 16):** verify one prior-introduced object is touched, used or noticed, OR one new plant-object is introduced for later. Check `plot/prestige-inventory.md` if it exists and `chapters/<book>/state.md` §Micro-details Planted. Neither: add one sentence of object interaction in the strongest scene.
14. **Silence beat (Rule 20):** a chapter containing a dramatic peak — revelation, betrayal, death, crossing, body blow — needs a silent beat: a white-space break, or a single short sentence isolated between longer paragraphs. Missing: insert one after the peak's physical moment.
15. **Forbidden interior-labeling formulas (HARD):** grep the narration (NOT dialogue). Each match is a candidate for cut or rewrite — express interior states through physical action, gesture or sensory tightening, never through narrator labels of feeling-words.
    - `the closest thing to <emotion> [he|she|they] had had in (days|weeks|years|months)` — ALWAYS cut. The preceding image is doing the work; the gloss betrays it.
    - `a kind of <abstract noun>` (e.g., "a kind of pleasure", "a kind of relief") — the concrete physical correlate, or cut.
    - `almost <verb>` / `almost felt like` — interior hedging. The gesture itself, or its interruption.
    - `started to <verb>` ... `[before|and] stopped`, where it labels an interior gesture rather than stating an exterior fact — the gesture or its bare interruption, no narrator commentary.
    - `<character> felt <X>` immediately followed by an explanation of what `<X>` is — collapse to the physical signal alone.
    Target: **0** matches in narration. These are forbidden regardless of check #6's count cap: that cap covers narrator wisdom, this covers narrator emotion-labeling, a distinct failure mode.
16. **External-world claim discipline (see `sniff.md` §5):** scan for every concrete assertion resting on knowledge outside the project's canon — real place names (streets, neighborhoods, landmarks, geography), specific technology (model numbers, connector types, hardware compatibility, software stacks, period plausibility), foreign-language grammar in proper nouns (article-preposition contractions, gender agreement, accents), real physics, biology, medicine, law, finance, brand names, dates, currencies, units. Classify each:
    - **(a) Project canon supports it.** Cite the canonical file (e.g., `world/level-0-reality/...`, `consumer-anchors.md`). Keep.
    - **(b) High-confidence real-world fact.** Declare it; the assertion is now load-bearing on real-world knowledge and is re-checked at sniff §5.
    - **(c) Cannot verify.** Abstract or cut.
    When in doubt, cut the specificity: false precision costs more than vagueness, and a generic "the cable" is worth more than a wrong specific one. The writer agent's tendency is to add plausible-sounding tokens for texture — that is the failure mode this check exists for, and `sniff.md` §5 is only the net.
17. **Physical/embodied plausibility (writer-side of Reviewer Rule 27; #16 audits factual *tokens*, this audits rendered *behavior*):** for every rendered physical action — a body under load, an object's mechanics (weight, friction, wear, heat, timescale), a chemical, biological or economic process — **pause and simulate it**: would this actually happen? A hard point under the foot's arch alters gait, because the arch descends under load; a ~10 W power brick warms over minutes, not in half a second. The failure mode is fluent prose written without running the physics, and it is a check you CAN do once you stop to simulate. **Level-aware:** at L0/Reality, full real-world fidelity; at L1/L2 the *substrate* is canon-governed — do not second-guess the rendered sky, light, time or material — but the *embodied human* is real, and a foot is a real foot. **Exempt — do NOT "fix":** deliberately-impossible planted hooks and mysteries (`plot/prestige-inventory.md`, `plot/reveal-hierarchy.md`) are SUPPOSED to break physics. **Do NOT explain the physics in the prose** (Rule 26, over-exposition): render the action quietly correct, or relocate the beat until it is. Post-draft net: Reviewer Rule 27.
...
Open **every file holding a `## Usage Tracker` row for THIS Book+Ch** — the set 2.6.c collected, re-derived here if the plan is stale. This is deliberately NOT "the files Step 1 loaded": a file with a row for this chapter that Step 1 never opened has a row that can neither be rendered nor ticked.

Where the two sets differ, report the difference in the Step 5.5 announcement instead of skipping it silently — the difference is a tracker↔context violation 2.6.c should have caught: *"⚠️ Step 5.5: N file(s) hold rows for this chapter but were not loaded in Step 1 (<list>) — tracker↔context gap; auto-add pending in the outline."* Rows in a level-barred file (the 2.6.c conflict) stay `planned` and are named in the same line.
