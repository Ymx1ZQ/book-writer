...
17. **Physical/embodied plausibility (the writer-side of Reviewer Rule 27 — distinct from #16, which audits factual *tokens*; this audits rendered *behavior*):** for every rendered physical action or behavior — a body moving/enduring/feeling under load, an object's mechanics (weight, friction, wear, heat, timescale), a chemical/biological/economic process — **pause and simulate it**: would this actually happen? If a body does something, would it really feel it / limp / strain (a hard point under the foot's arch alters gait — the arch descends under load); if an object behaves, are the magnitude and timescale real (a ~10 W power brick warms over minutes, not in half a second). The failure mode is writing fluent prose without running the physics — this is a knowledge+reasoning check you CAN do when you stop to simulate. **Level-aware:** at L0/Reality, full real-world fidelity; at L1/L2 (Dome/Ark = simulations) the *substrate* is canon-governed (don't second-guess the rendered sky/light/time/material) but the *embodied human* is real (a foot is a real foot). **Exempt — do NOT "fix":** deliberately-impossible planted hooks/mysteries (`plot/prestige-inventory.md`, `plot/reveal-hierarchy.md`) are SUPPOSED to break physics. **Do NOT explain the physics in the prose** (that is over-exposition — Rule 26): render the action so it is *quietly* correct, or relocate/adjust the beat until it is. Post-draft net: Reviewer Rule 27.
