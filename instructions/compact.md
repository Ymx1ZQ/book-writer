...
**C. PRESERVE Usage Trackers.** Do NOT remove or modify Usage Tracker sections — they are writing scaffolding. Each tracker is a checklist ensuring every character/concept detail gets used at least once and is not repeated. Empty checkboxes `- [ ]` are the POINT — they track what has NOT yet been written. Items are marked `[x]` ONLY by `/book chapter` after a chapter is written and verified. Leave all trackers intact. This applies to character files, world-concept files (the-word.md, nothingness.md, micro-details.md, waiting.md), and plot files (key-scenes.md).
