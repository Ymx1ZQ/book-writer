...
Audit the chapter's recurring symbols and motifs against the project's own motif canon, for the one defect no other pass owns: a motif used **inverted**, **drifting**, or **arriving without its meaning** — the symbol that the reader's accumulated associations make *mean* something the prose did not intend.
...
None of those tracks whether a motif's **semantic charge** is consistent — whether the moka pot that meant *home* in ch01 is accidentally inverted into *threat* in ch03 without the text earning the reversal. That gap is this check.
...
1. `plot/motif-tracking.md` — the canonical registry and source of truth: each motif, its established meaning, intended evolution across books/chapters, and payoff plan. If it does not exist or is thin, that is itself a canon finding (route to DEVPLAN: "motif registry under-specified").
...
4. Prior chapters' use of each motif (the earlier `chapters/<book>/chNN.md` occurrences, or the snapshots in `chapters/coldread-state/`) to know the **accumulated** reader association entering this chapter.
   **Graph-fresh path (optional — see `instructions/graph-recall.md`):** if `graphify-out/graph.json` exists AND the freshness gate passes, bound this re-read: for each motif present in the target chapter, run `graphify query "<motif> established meaning, instances by chapter, intended evolution"` and `graphify path "<motif>" "<payoff-chapter>"`, then re-read from disk ONLY the prior-chapter §§ the graph cites (index mode — the disk text is the truth). Graph absent or stale → the unbounded re-read above stays the fallback.
...
1. **Inversion (BLOCKING-grade if unearned).** The instance carries a charge **opposite** to the established meaning, and the text does not earn the reversal: to a reader carrying the association, a symbol of safety used in a threatening beat reads as a mistake or an unsignaled irony. Earned reversal (the text deliberately turns the symbol, and signals the turn) is SAFE-KEEP — note it as intentional.
...
3. **Evolution mismatch.** The tracker specifies an intended evolution (e.g., the five-note motif should harden from *trill* → *scratch* by Book 2); the chapter either fails to evolve it (still the old form when it should have moved) or evolves it in a direction/at a pace the tracker did not intend. Route: prose wrong → SMELL; *tracker* the thing that should change (the chapter's evolution is better and canon should ratify it) → DEVPLAN.

4. **Payoff misalignment / unprepared payoff.** A motif's payoff beat fires but the setup instances in this (or prior) chapters did not prepare its specific charge, so the payoff lands flat or unearned; OR a setup instance is doing payoff-weight work too early, spending the charge before the planned payoff. The *semantic* complement to coherence's Chekhov pass: the gun is fired, but does the firing *mean* what the setups promised?
...
**Calibration (load-bearing).** Only flag when the misuse is something a careful reader holding the accumulated association would actually register as wrong — not every resonance you can construct. An ambiguous symbolic reading is SAFE-KEEP, never a flag. The cost of flattening a deliberate symbolic turn is high.
...
4. Append (never overwrite, per `readability.md` Step 5 rule) SMELL.md entries (`Source: motif`) + the Motif Coherence Audit section.
...
- **Least trigger-happy detector.** Symbolism is interpretive; this check must default to SAFE-KEEP.
- **Earned reversals are good.** A signaled turn of a symbol is craft, not a defect — say so in the audit.
