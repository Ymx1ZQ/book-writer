...
Arguments: `<book>` (book-1, book-2, book-3), optionally `<chapter>` (ch01, ch02, ...) — `/book review book-1 ch03` reviews a single chapter.
...
**Pre-step archive (Phase 9 M4):** before writing any new REVIEW.md, if `chapters/<book>/REVIEW.md` already exists, rename it to `chapters/<book>/archive/REVIEW-<YYYYMMDD-HHMMSS>-<chapters-reviewed>.md` (creating the archive subdir if needed). Preserves per-cycle review history.

### 2. Check Existing Reviews & Read the Chapters

**Before reading chapters, check `chapters/<book>/REVIEW.md`.** If it exists, parse the `**Reviewed:**` line to determine which chapters have already been reviewed. A chapter is "already reviewed" if:
...
- If no chapter specified: read ALL existing chapter files in `chapters/<book>/ch*.md` (excluding plan files), then **exclude** chapters already reviewed or with corrections pending. From the remaining unreviewed chapters, **cap the session at 10**. If more than 10 unreviewed chapters exist, take the first 10 in chapter order and announce how many remain for the next session.
...
- **Rule 26 — Exposition economy (no restatement, no self-evident justification, no over-run):** the **trust-the-reader regularizer** — the counter-pressure to the coldread comprehension gradient. It fires ONLY where a cut preserves a *first-time* reader's comprehension. Scan for three sentence-/paragraph-level inertia patterns and quote each offender: **(a) Restatement** — a fact, rule, or stake already established earlier in THIS chapter, re-said with no new content (e.g. a consequence re-named two paragraphs after it was named; a threshold stated twice). **(b) Self-evident justification** — the narrator explaining *why* something the reader already infers from the immediate context is so (a character's routine action justified; a non-action given ≥2 reasons; a routing/assignment explained when the scene already implies it). **(c) Over-run** — a hypothetical / consequence / projection that continues past its landing line (a dread-syllogism rendered in N sentences when the button lands in 1–2), or an over-clever convolution that loops a word to make a point (e.g. `the previous one if the previous one had not been the previous one`). **GUARDS — do NOT flag if any holds:** removal would cost a *first-time* reader a necessary fact (the load-bearing guard against gutting legitimate onramp/setup); or the span is (i) the **first occurrence** of a world-rule in onramp position (early chapter teaching the reader the rule), (ii) a planted setup/payoff (`plot/prestige-inventory.md`), (iii) a voice-floor pillar (`writing-notes.md`), (iv) dramatic irony or thematic payload stated **once** (a single statement of the central irony is earned; its *restatement* is not), or (v) a POV expert perceiving in-character — verb-mode perception (Lena reading a graph and clocking why it is anomalous) is PASS, distinct from narrator subject-mode exposition. **Classify:** pattern (a) pure restatement → **SAFE-CUT** (the content survives in its first statement, no loss); patterns (b)/(c) → **TRADE-OFF** (a clause is subtracted; arbiter adjudicates). `Suggested action` quotes the redundant / self-evident / over-run span and names what survives — *"cut to the established statement"* / *"keep the button, drop the run-up"* — never a vague "tighten." Severity MEDIUM. **Boundary (do not duplicate):** distinct from Rule 24 (narrator explaining HOW a mechanism works — subject-mode mechanism exposition; Rule 26 is redundancy / self-evidence / over-run regardless of whether a mechanism is involved), Rule 25 (within-chapter *beat* recurrence — Rule 26 is *sentence/paragraph* expository wordiness, single occurrence), and Rule 19 (cross-chapter motif echo). Test that separates Rule 26 from coldread: coldread asks "would a reader stumble for lack of setup?"; Rule 26 asks "would a reader notice nothing missing if this were cut?" — fire only when the answer is yes.
- **Rule 27 — Reality plausibility (the fourth pillar):** the pipeline checks fidelity to the *text* (craft) and to the *book's own rules* (canon); Rule 27 checks fidelity to **how the real world actually works**. Ask of a concrete, checkable, load-bearing claim: does it hold against real physics / chemistry / biology / economics / society / technology? **LEVEL-SCOPING (load-bearing — get this wrong and you flag the wrong things):** **L0 / Reality (2045)** → full real-world fidelity, extrapolated forward. **L1/L2 (Ark / Dome = simulations)** → the *substrate* (ambient physics, light/time, material behavior) is CANON-governed; do NOT flag a simulated-world rule as "unrealistic" (the Algorithm sets it). BUT the **embodied-human layer is real** — the simulated people have real bodies and minds, so biomechanical / physiological / somatic / psychological / social plausibility still applies (Roe's foot is a real foot; the medial arch descends under load whether or not the sky is rendered). **LENSES — flag only SPECIFIC, VERIFIABLE, LOAD-BEARING claims:** physical/mechanical (biomechanics, materials, forces, thermodynamics/heat), chemical (reactions, toxicity, drugs/suppressants, air/food), biological/physiological/neuro (a body/brain's adaptation, stress markers, sleep, aging, nutrition, the implant's mechanism), economic (prices, incentives, markets, energy/resource flows — cross-check `world/level-0-reality/consumer-anchors.md`, `economy.md`), social/institutional (how bureaucracies, crowds, trust-networks, black markets actually behave), technological/computational/AI (feasibility of compute/network/agent capability — cross-check `world/ai-technical-foundation.md`, `world/level-0-reality/agent-capabilities.md`), logistical/temporal (can the event happen in the time/space given). For each candidate: **quote the claim, name the lens + level, state the concrete real-world fault (the *why* — e.g. "the medial arch descends under load; a hard point there concentrates pressure → gait alteration"), and propose a fix that preserves the beat.** Severity HIGH if the falsehood is load-bearing or an expert reader would catch it; MEDIUM if minor/peripheral. **GUARDS — do NOT flag if:** (i) the claim is at L1/L2 and concerns the *simulated substrate* (canon-governed physics/time/material); (ii) it is a **deliberately-impossible planted hook/mystery** (`plot/prestige-inventory.md`, `plot/reveal-hierarchy.md` — e.g. the offline device that knows Noah is *supposed* to be impossible; flagging it is the opposite error); (iii) an established worldbuilding file canon-overrides it (the in-world rule wins); (iv) it is metaphor/figurative, not a literal claim. Apply the **highest** bar to the social/psychological lenses (fuzzy → only egregious, specific, checkable; subtle motivation/emotion is beat-craft, out of scope). **Classification:** unlike Rule 26 (mechanical trim), Rule 27 findings are CORRECTIONS needing **authored revision** (relocate / acknowledge the cost / re-number / cut) — route to the review devplan as fix-items, NOT arbiter auto-trims. **Boundary:** distinct from canon-check (internal consistency); Rule 27 is *external* reality-fidelity — "is this true to how X actually works?", not "is it consistent with the book?". Honest note: this is largely a knowledge+reasoning check the model CAN do when explicitly directed (high recall on economics/known-physics/chemistry/compute-feasibility), weaker on fine biomechanics and 2045-extrapolation — so reason each claim out explicitly, do not pattern-match.
- **Rule 28 — Register-lock compliance (review-time net for `world/register-locks.md`):** `register-locks.md` is a WRITE-TIME constraint (loaded at `chapter-writer.md` Step 1); Rule 28 is its REVIEW-TIME net. Determine the chapter's level (Reality L0 / Dome / Ark), load `world/register-locks.md`, and scan the prose for that level's **forbidden patterns**, quoting each hit + the `register-locks.md` line it violates. For **Reality (L0)**: (a) **"X was the X" tautologies** (`the cursor sat where the cursor sat`, `the cord was where the cord was`, `the grit was grit`) — Dome-narrator tics; (b) **apophatic chains of 4+** consecutive negated sentences (`He/She did not / had not / was not / would not ___`) (Dome signature); (c) **Dome compliance-lexicon** in narration (`within parameters`, `compliance nudge`, `deniable band`, `recalibrated`, `cycle N`, `shadow phase`). For **Dome / Ark**: their own forbidden lists per `register-locks.md` (e.g. Mediterranean warmth / sea-wind / espresso / market vendors = Reality, forbidden in Dome). Severity **HIGH** — a leak collapses the per-level register distinction the trilogy's structure depends on. Classification: authored fix (rewrite the leaked phrase in the chapter's own register — e.g. tautology → a concrete statement of what is actually happening). **GUARD:** Book-3 convergence chapters where the three registers *intentionally* bleed (`world/tones.md` — B3 only) are exempt. **Boundary:** mechanical/grep-able and level-scoped; distinct from Rule 27 (reality-fidelity) and coldread (comprehension). This rule exists because the leak that prompted it (`the cursor sat where the cursor sat`, ch04 Reality) survived precisely because nothing re-scanned finished prose against the register locks.

### 4. Cross-Chapter Analysis (batch review only)

If reviewing multiple chapters:

- **Voice drift:** Do the characters' voices stay distinct across chapters? Does any POV character's voice bleed into the narrator's?
...
After the report, create (or update) the file `chapters/<book>/REVIEW.md`. This is a **correction devplan** — an ordered checklist of every SAFE-CUT fix the reviewer identified.
