...
| **"Can the reader parse this sentence on one pass?"** — referential / state / jargon friction (category 11) | **THIS check** |

Sniff is the **plausibility / nose-wrinkle** layer. It catches the patterns a writer agent typically falls into when worldbuilding is silent: filling gaps with plausible-but-uncanonicalized invention, modern unconscious bias, technobabble without referent, unearned capabilities, convenient coincidences. Category 10 extends the same reader-friction lens to *prose-event mannerism*; category 11 extends it again to *sentence-level legibility* — whether a reader can parse a sentence on a single linear pass at all.
...
## The eleven objection categories

Categories 1-9 and 11 are evaluated **line-by-line**: for every concrete assertion in the chapter (a number, a fact, an object, a capability, a behavior, a coincidence), check it against all ten line-level categories. Most lines will pass. Category 10 is evaluated **device-by-device, then occurrence-by-occurrence**: it audits recurring prose devices (tautology, anaphora, fragments, parallel triplets, etc.), not individual assertions.

**Flagging discipline (Phase 9 M2 — read before scanning).** An objection that triggers one of the eleven categories below is a *candidate*, not a verdict. Before adding to SMELL.md, every candidate must pass the same three-question test the Reviewer applies:
...
Three-tier flagging classification:
...
5. **Domain plausibility.** Read each assertion as if the reader has expert knowledge in the relevant domain — economist, engineer, doctor, person with the relevant disability, native of the city/region, professional in the depicted field. Would they call BS? "Two euros the kilo" for fresh sardines in a 2045 climate-stressed Mediterranean = an economist calls BS. A wheelchair maneuver that cannot physically be done with the chair geometry described = a wheelchair user calls BS. A surgical detail wrong by current medical practice = a doctor calls BS.
   ...
   **5.a — Real-world factual-claim audit (operational sub-rubric).** For every concrete assertion in the chapter that depends on knowledge **external to the project's canon files**, classify into one of three buckets:
...
10. **Stylistic excess (form > function).** A *device* is a stylistic technique repeated across the chapter: tautology (X was X), anaphora, asyndeton, sentence fragments, parallel triplets, structural repetition of openings/closes, nominal sentences, catalog sentences, etc. The failure mode is: the form is louder than the function it was supposed to render. The reader notices the *writing* instead of the world/character — "this author is showing off, this is too much". This is distinct from categories 1-9 (factual/world plausibility) and from Reviewer (scene shaping, pacing): it is **prose-event mannerism**.
   ...
   **10.f — Device saturation (the one licensed wholesale finding).** §10.a–§10.e govern *per-occurrence* flagging; by design they cannot raise "device X carries the whole chapter," and that gap let a monotone chapter ship clean. From the §10.a metrics, a device is **saturated** when its **Count ≥ 12** OR its **max 50-line-window density ≥ 6**. (Defaults — calibration-tunable; the target is: a device supplying the chapter's dominant texture triggers, a device used as a deliberate but bounded motif does not.) For each saturated device, raise **exactly one** chapter-level `SATURATION` finding, separate from and additional to the §10.d per-occurrence budget. The `SATURATION` finding:
   ...
   **Prevention layer.** `chapter-writer.md` §3.5 does not currently include a stylistic-device audit — Category 10 is the only line of defense.

11. **Sentence-level reader friction.** Categories 1-9 ask whether the reader *believes* a line; Category 10 whether the writer is showing off; this category asks the prior question — whether the reader can *parse the sentence at all*, on one linear pass, in order, without the whole-chapter context the executing agent has and the reader does not. This is the class an LLM reviewer is structurally worst at: it comprehends holistically, so it silently resolves ambiguity, supplies missing preconditions, and decodes jargon the reader cannot — it reads cooperatively. The defense is to stop *judging* and start *enumerating*: list the items, then test each mechanically. Evaluated **line-by-line** alongside 1-9. Three enumerate-then-flag sub-rubrics. For all three: **do not be charitable** — if a sentence has a reading on which it is unclear, unestablished, or opaque, report that reading as if it were the only one. Do not construct the author's intended meaning; do not supply a precondition the text never gave.

   **11.a — Referential ambiguity.** For every pronoun and every definite reference ("the man", "his game", "that one") whose antecedent is not the immediately obvious subject, list every grammatically possible antecedent. If more than one is plausible and the sentence does not disambiguate, flag — a reader committing to an interpretation as they go cannot recover. (Example: "no surname for himself" — *himself* could be the contact named on the line above or the notebook's writer; the reader cannot tell which.)

   **11.b — Unestablished physical state.** For every asserted physical state of an object or a body (warm, cold, wet, lit, running, loud, still), name its precondition — what the world must have done for that state to hold (a warm device was powered, or recently powered, or sat in heat). Verify the precondition is established in the text *before* this point. If it is not, flag — do not supply the precondition yourself. (Example: "the case was warm" on a device the prose has not yet switched on.)

   **11.c — Jargon opacity.** For every technical or system term, ask whether a non-expert reader decodes it *at reading speed*. This is distinct from Category 3: there the test is whether the system exists in canon; here the system may be perfectly canonical and the sentence still unreadable. If the phrase cannot be parsed on one pass by a reader without the domain, flag. (Example: "no isolated consumer-level delta visible above meter noise" — canonical mechanism, opaque sentence.)

   Routing is almost always INLINE (a prose fix). 11.a and 11.c are rarely voice-floor, so they are usually SAFE-CUT; 11.b can be TRADE-OFF when the unestablished state is a deliberate withhold.
...
After the per-objection entries (and before any "Acknowledged" SAFE-KEEP block), include a section listing every stylistic device identified in the chapter, even if zero occurrences flagged — the transparency artifact of §10.e.
...
2.5. **Graph-assisted claim-driven canon lookup (optional — see `instructions/graph-recall.md`).** If `graphify-out/graph.json` exists in the project root AND the freshness gate passes (`graph-recall.md` §Opt-in detection + §Freshness gate), SKIP the wholesale level-directory and character-sheet load in Step 2 and resolve canon claim-by-claim during the Step 7 scan instead — index mode: the graph names file §§, the disk read is the truth.
   - **Per suspicious assertion** (a candidate under categories 1-4 and 8): run `graphify query "<asserted fact> canon status" --budget 4000` → read only the file §§ the result points to, from disk.
   - **Category 7 (character behavior):** graph triage first — query the character's established constraints (voice, defaults, history, relationships); open the character sheet ONLY on a hit (the query surfaces a constraint the scene's behavior may contradict), and then read just the pointed-to §§ from disk.
   - **NEVER substituted:** §5/§5.a plausibility checks keep their verbatim anchor-value reads — the exact price / frequency / allocation figure comes from the anchor file itself (e.g. `consumer-anchors.md`), never from a graph node's summary (never-substitute list per `graph-recall.md`). `chapters/<book>/writing-notes.md` (Step 6 voice-floor pass + Category 10 coverage) stays a full read. Steps 3-4 (`timeline.md`, `canon-hierarchy.md`) load unchanged.
   - **Fallback:** graph absent or stale → this step is a no-op and Step 2 proceeds exactly as written. An empty query result is never evidence of absence — load the relevant level files for that assertion as Step 2 specifies (`graph-recall.md` §Fallback ladder).
...
5. **Pre-step archive (Phase 9 M4):** if `chapters/<book>/SMELL.md` already exists, rename it to `chapters/<book>/archive/SMELL-<YYYYMMDD-HHMMSS>-<chapter>.md` (creating the archive subdir if needed) BEFORE writing the new one. Per-cycle finding history preserved.
...
7. Read the chapter line by line. For every concrete assertion, run categories 1-9 and 11 (if the Step 2.5 graph path is active, resolve each assertion's canon status via the per-claim queries there instead of pre-loaded level files). For Category 10, audit per-device using the metrics from Step 6.5 (not line-by-line) — apply the §10.b protections and §10.c per-occurrence criteria, respecting the §10.d budget (1-3 occurrences per device max). Then apply §10.f: test every device against the saturation threshold and raise one `SATURATION` finding per saturated device. Aggregate candidate findings.
8. **For each candidate, apply the three-question test from §"The eleven objection categories" preamble** — articulate Improvement, articulate Loss, check Voice-floor — then assign Flagging (SAFE-CUT / TRADE-OFF / SAFE-KEEP) AND Routing (INLINE / ANCHOR-NEEDED / ACCEPT). Include both classifications in the entry.
9. Write `chapters/<book>/SMELL.md` with the format above. Include all three flagging tiers (SAFE-CUT / TRADE-OFF / SAFE-KEEP) — SAFE-KEEP entries are noted in an "Acknowledged" block at the end of SMELL.md without action. **Always include the "Stylistic Device Audit" section** (Category 10) per §10.e, even if zero occurrences flag.
...
- **Category 10 is device-by-device, then occurrence-by-occurrence — with one licensed wholesale exception.** Per-occurrence flags need a specific articulable failure (first-encounter friction, internal density overflow, form > function); devices named in `writing-notes.md` and setup occurrences under a payoff break are protected per §10.b, max 1-3 per device per §10.d. The exception is the §10.f `SATURATION` finding: when a device's Count or window-density crosses the saturation threshold, raise exactly one chapter-level finding for it. That finding IS wholesale, by design — it is the check's only defense against a single device supplying the chapter's entire texture, and it is not silenced by writing-notes coverage or by voice-floor protection.
...
- The `SMELL.md` file is per-chapter. A re-run archives the previous one to `chapters/<book>/archive/` (Step 5) before writing the new one, so per-cycle history survives outside git history too.
